...
document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("loginForm").addEventListener("submit", async function (e) {
        ...
        try {
            // Realizar la solicitud al backend para autenticar al funcionario
            const response = await fetch(`http://localhost:8080/funcionario/buscarPorRut/${rut}/${password}`);
            
            if (response.ok) {
                const funcionario = await response.json();
                // Almacenar los datos del funcionario en localStorage (si es necesario)
                localStorage.setItem("funcionario", JSON.stringify(funcionario)); 
                
                Swal.fire({
                    icon: 'success',
                    title: '¡Bienvenido!',
                    text: `Hola ${funcionario.nombre}, has iniciado sesión correctamente como funcionario.`,
                    showConfirmButton: false,
                    timer: 2000 // Ocultar automáticamente después de 2 segundos
                }).then(() => {
                    // Redirigir a la página de inicio de funcionarios
                    window.location.href = "/home/funcionario_home.html"; // Asegúrate de que esta sea la ruta correcta
                });
            } else {
                ...
                Swal.fire({
                    ...
                    text: errorMessage || "Usuario o contraseña de funcionario incorrectos." // Muestra el mensaje del backend o uno por defecto
                }).then(() => {
